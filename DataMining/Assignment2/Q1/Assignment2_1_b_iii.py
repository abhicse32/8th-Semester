...
def quick(lst,min_sup,top_X):
    ...
    top_list=sorted(frequent_set,key=frequent_set.get,reverse=True)
    len_=len(top_list)

    top_list=top_list[:int(top_X*len_)]
    for items in top_list:
        frequent_set_top[items]=frequent_set[items]
    ...
    for items in lst:
        temp=[]
        for item in items:
            if item in frequent_set_top:
                temp.append(item)
        lst2.append(temp)
    print (len(lst2))
    frequent_set=frequent_set_top
...
for percent in fraction:
    ...
    C,mod_trans_id,min_sup_count=quick(lst[:int(percent*len_lst)],min_sup,percent)
