...
for k in range(11):
    ...
    min_sup=0.002+0.001*k
    C,mod_trans_id,min_sup_count=quick(lst,min_sup)
    i=1
    ...
    ################################################################################################
    total_time=time.time()+start - start_time
    ...
    for frequent_items_ in sorted_frequent_items:
        print frequent_items_[0],len(frequent_items_[1])
    print

    min_sup=min_sup*100
    #print ("[%f ,%s],")%(min_sup,total_time)
    #print sorted_frequent_items

    with open("CS12B032_1b_i.txt","a") as outfile:
