...
def find_frequent_itemsets(transactions, minimum_support, include_support=False):
    ...
    #transactions=transactions[:19876]
...
if __name__ == '__main__':
    ...
    try:
        for i in range(11):
            ...
            f = open(args[0])
            temp=0.002+.001*i
            min_sup=temp*options.minsup
            for itemset, support in find_frequent_itemsets(csv.reader(f), min_sup, True):
                size= len(itemset)
                if size not in x:
                    x[size]=0
                x[size]+=1
            print x
            #print "[%f,%f]"%(temp*100,time.time()-start)
                #print '{' + ', '.join(itemset) + '} '
            #x=189
    finally:
        f.close()
