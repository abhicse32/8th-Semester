...
def find_frequent_itemsets(transactions, minimum_support,percent,include_support=False):
    """
    ...
    just the itemsets.
    """
    transactions=[tran for tran in transactions]
    shuffle(transactions)
    transactions=transactions[:percent]
...
if __name__ == '__main__':
    ...
    data_=[.1,.25,.75,1.0]
    min_sup=.002*options.minsup
    try:
        for i in data_:
            start=time.time()
            f = open(args[0])
            
            for itemset, support in find_frequent_itemsets(csv.reader(f), min_sup,int(i*options.minsup),True):
                min_sup
            print "[%d,%f],"%(i*100,time.time()-start)
                #print '{' + ', '.join(itemset) + '} '
            #x=189
    finally:
        f.close()
